...
for i in eachindex(deltas)
    ...
    wt_df_historical = CSV.read("./$(variable)/wt_$(period_historical)/wt_$(variable)_delta_$(delta).csv", DataFrame)
    ...
    sc12[i] = scatter!(ax1, multiplier[i] .* x_historical, y_historical, color=(:gray, 0.65), marker=:circle, markersize=5)
...
# AXIS LEGEND
axislegend(ax1, [sc1[i] for i in eachindex(deltas)], [L"\delta=%$(deltas[i]) \, \mathrm{[cm]}" for i in eachindex(deltas)], position = :rt, bgcolor = (:grey90, 0.25), labelsize=18, titlesize=18);
...
save("./results_article_plots/wt_$(variable)_best_fits_and_historical.png", fig, px_per_unit=7)
fig

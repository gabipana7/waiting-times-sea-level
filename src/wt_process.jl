...
for delta in deltas
    ...
    save("./$(variable)/results/$(period)/wt_$(variable)_delta_$(delta).png", fig, px_per_unit=5)

end

# results = DataFrame([deltas, wt_alpha, wt_sigma, wt_xmin, wt_KS], ["delta", "alpha", "sigma", "xmin", "KS"])
CSV.write("./$(variable)/results/$(period)/results_$(variable).csv", results, delim=",", header=true);
